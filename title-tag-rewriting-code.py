...
st.set_page_config(page_title="Luke's H1 & Title Tag Rewriter", page_icon="✍️", layout="wide")
...
# --- 1. Cached File Loader (Prevents Freezing/Stalling) ---
...
PROTECTED_WORDS = {
    "x", "w", "d", "h", "cm", "mm", "kg", "&", "and", "or", "in", "for", "of", "with", "|", "-"
}
...
def parse_url_taxonomy(url_str, curr_title="", curr_h1=""):
    ...
    clean_curr_title = curr_title if curr_title and str(curr_title).strip() != "0" else ""
    full_context = f"{full_path_str} {clean_curr_h1.lower()} {clean_curr_title.lower()}"
    ...
    for seg in path_segments:
        ...
        if re.search(r"\d+wx\d+h", seg_lower) or re.search(r"\d+wx\d+dx\d+h", seg_lower):
            ...
            if d3:
                dimensions = f"{d3.group(1)}w x {d3.group(2)}d x {d3.group(3)}h cm".upper()
            elif d2:
    ...
    if product_type:
        if product_type.lower() not in current_title_str.lower():
            if "door" in current_title_str.lower() and product_type.lower() in ["lockers", "door lockers"]:
...
# --- Helper 4: Disambiguation Engine ---
def ensure_unique_title(url_str, base_h1, brand_suffix, max_len, seen_titles):
    ...
    parsed = urlparse(url_str)
    segments = [s.replace("-", " ").title() for s in parsed.path.split("/") if s and not s.endswith(".html")]

    differentiating_h1 = base_h1
    for seg in reversed(segments[:-1]):
        diff_words = [
            w for w in seg.split()
            if w.lower() not in base_h1.lower()
            and w.lower() not in ["lockers", "tables", "desks", "chairs", "metric", "furniture"]
        ]
        if diff_words:
            diff_text = " ".join(diff_words)
            differentiating_h1 = deduplicate_title_words(f"{diff_text} {base_h1}")

            candidate_title = f"{differentiating_h1}{brand_suffix}"
            if len(candidate_title) > max_len:
                max_body_len = max_len - len(brand_suffix)
                truncated_body = differentiating_h1[:max_body_len].rsplit(" ", 1)[0]
...
if uploaded_file is not None:
    ...
    with col1:
        url_col = st.selectbox(
            "URL / Address Column",
            df.columns,
            index=df.columns.get_loc(default_url_col) if default_url_col in df.columns else 0,
        )

    with col2:
        title_index = df.columns.get_loc(default_title_col) + 1 if default_title_col in df.columns else 0
        title_col = st.selectbox("Current Title Column", column_options, index=title_index)

    with col3:
        h1_index = df.columns.get_loc(default_h1_col) + 1 if default_h1_col in df.columns else 0
        h1_col = st.selectbox("Current H1 Column", column_options, index=h1_index)
    ...
    if st.button("Generate Rewritten Titles & H1s"):
        with st.spinner("Processing titles..."):
            ...
            for index, row in df.iterrows():
                ...
                url = str(row[url_col])
                current_title = str(row[title_col]) if (title_col != "None" and title_col in df.columns) else ""
                current_h1 = str(row[h1_col]) if (h1_col != "None" and h1_col in df.columns) else ""

                raw_h1 = parse_url_taxonomy(url, curr_title=current_title, curr_h1=current_h1)
                final_h1, final_title = ensure_unique_title(url, raw_h1, brand_suffix, max_title_len, seen_recommended_titles)
